...
module.exports = function(app) {
	app.post('/addfiles', upload.any(),  (req, res)=>
	{
		...
		{
			var temp = {};
			temp['fname'] = mime.lookup(req.files[i].originalname).split('/')[0]+'/'+req.files[i].originalname;
			files.push(temp);

			console.log("Uploading : "+req.files[i].originalname);

			/* uploading in s3 */
			var userFolder = 'drop.io/'+req.body.drop+'/'+mime.lookup(req.files[i].originalname).split('/')[0];
